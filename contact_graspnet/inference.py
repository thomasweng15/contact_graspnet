...
def inference_from_file(global_config, checkpoint_dir, input_paths, K=None, local_regions=True, skip_border_objects=False, filter_grasps=True, segmap_id=None, z_range=[0.2,1.8], forward_passes=1):
    """
...
def inference(sess, grasp_estimator, x, K=None, local_regions=True, skip_border_objects=False, filter_grasps=True, segmap_id=None, z_range=[0.2,1.8], forward_passes=1, visualize=False):
    """
    Predict 6-DoF grasp distribution for given model and input data
    ...
    """
    rgb = x['rgb']
    depth = np.squeeze(x['depth'])
    cam_K = x['K']
    segmap = np.squeeze(x['seg'])
...
if __name__ == "__main__":
    ...
    inference_from_file(global_config, FLAGS.ckpt_dir, FLAGS.np_path if not FLAGS.png_path else FLAGS.png_path, z_range=eval(str(FLAGS.z_range)),
